...
def trainModel():
    ...
    train = np.array([plt.imread(join(dirname(__file__),('Data/Train/'+str(i)+'.jpg'))).reshape(-1) for i in range (1,2401)])
    train_labels = np.loadtxt(join(dirname(__file__),'Data/Train/Training Labels.txt'))
    ...
    np.savetxt(join(dirname(__file__),'Data/MeanStd/mean.csv'), save_mean)
    np.savetxt(join(dirname(__file__),'Data/MeanStd/std.csv'), save_std)
...
def testImage(im, qLevel):
    #importing mean and std
    loaded_mean = np.loadtxt(join(dirname(__file__),'Data/MeanStd/mean.csv'))
    loaded_std = np.loadtxt(join(dirname(__file__),'Data/MeanStd/std.csv'))
    mean_qsp = loaded_mean.reshape(loaded_mean.shape[0],loaded_mean.shape[1] // 784, 784)
    std_qsp = loaded_std.reshape(loaded_std.shape[0],loaded_std.shape[1] // 784, 784)

    #Adjusting image to match training dataset
    #im = cv2.imread(join(dirname(__file__),'Data/Individual/T2.jpeg'))
    image = stringToRGB(im)
    #image = cv2.cvtColor(tempIm, cv2.COLOR_BGR2GRAY)
    dimensions = image.shape
    # height, width, number of channels in image
    height = dimensions[0]
    width = dimensions[1]
...
#prediction = testImage(cv2.imread('Data/Individual/T3.jpeg'), 2)

#print('The number in your image is: '+str(prediction[0]))

def testPython():
    return 'Chaquopy is working correctly'